...
describe("AdvancedGuardianRecovery", function () {
    ...
    beforeEach(async function () {
        ...
        TokenboundAccount = await deployments.get("TokenboundAccount");
        tokenBoundAccount = await ethers.getContractAt("TokenboundAccount", TokenboundAccount.address, owner);
        ...
        AdvancedGuardianRecovery = await deployments.get("AdvancedGuardianRecovery");
        advancedGuardianRecovery = await ethers.getContractAt("AdvancedGuardianRecovery", AdvancedGuardianRecovery.address, owner);
    });

    it("should allow owner to add and remove guardians", async function () {
        await advancedGuardianRecovery
          .connect(owner)
          .addGuardian(owner.address);
        expect(await advancedGuardianRecovery.connect(owner).isGuardian(owner.address)).to.be.true;

        await advancedGuardianRecovery.connect(owner).removeGuardian(owner.address);
        expect(await advancedGuardianRecovery.connect(owner).isGuardian(owner.address)).to.be.false;
    });
